...
cypher_generator = LLMChain(
    llm=llm,
    prompt=cypher_prompt.partial(schema=schema)
)

# --- 4) NL conversion prompt ---
nl_prompt = PromptTemplate.from_template(
    """Ignore the subject name and convert the Cypher Answer 
    into a short and direct natural answer that answers the question clearly.
Question: {question}
Cypher query: {cypher}
Raw result: {result}
...
nl_converter = LLMChain(llm=llm, prompt=nl_prompt)

# --- 5) Helper to clean Cypher ---
...
st.write("Ask a question about subjects, prerequisites, or outcomes.")
...
if st.button("Submit"):
    ...
    # Generate Cypher
    cypher = cypher_generator.run(question)
    ...
    # Convert to NL
    if rows:
        nl_answer = nl_converter.run({
            "question": question,
            "cypher": cypher_query,
            "result": rows
        })
    else:
        nl_answer = "I could not find any matching subjects."
    ...
    st.caption(f"⏱️ Answered in {elapsed} seconds")